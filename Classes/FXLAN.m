...
- (void)cancelConnect{
    self.host = true;
    if (self.invitePeer) {
        [self.currentSession cancelConnectPeer:self.invitePeer];
    }
    [self stopBrowser];
}
        ...
        if (peerID == self.invitePeer) {
            switch (state) {
                case MCSessionStateNotConnected:
                    NSLog(@"对方未连接");
                    self.currentState = FXLANConnectStateNotConnected;
                ...
                case MCSessionStateConnecting:
                    NSLog(@"正在建立连接");
                    self.currentState = FXLANConnectStateConnecting;
                    if (self.delegate !=nil && [self.delegate respondsToSelector:@selector(refreshConnectState:)]) {
                ...
                case MCSessionStateConnected:
                    NSLog(@"对方已连接");
                    self.currentState = FXLANConnectStateConnected;
                    if (self.delegate !=nil && [self.delegate respondsToSelector:@selector(refreshConnectState:)]) {
...
- (void)session:(MCSession *)session didReceiveCertificate:(NSArray *)certificate fromPeer:(MCPeerID *)peerID certificateHandler:(void (^)(BOOL))certificateHandler{
    dispatch_async(dispatch_get_main_queue(), ^{
        self.currentState = FXLANConnectStateFirst;
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(refreshConnectState:)]) {
